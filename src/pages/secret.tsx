...
/**
 * Viewer for One-Time secrets created via the `ots@0.0.9` CLI.
 *
 * @deprecated Intended for backwards compatibility support only.
 * New versions of OTS print URLs that point to `/view/:secretId`.
 */
const ViewOtsFromOtsCliScreen = () => {
  ...
  const { id: secretId, v: version } = query as { id: string; v: string }
  ...
  useEffect(() => {
    ...
    async function readSecret() {
      try {
        const baseUrl = version ? "api.ots.sniptt.com" : "apiv2.beta.snipt.io"

        const response = await fetch(`https://${baseUrl}/secrets/${secretId}`, {
          headers: {
        ...
        const { EncryptedBytes: encryptedBytes }: OneTimeSecret =
          await response.json()
...
export default withNoSsr(ViewOtsFromOtsCliScreen)
