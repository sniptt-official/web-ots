...
/**
 * Viewer for One-Time secrets created via the `snip@0.0.51` CLI.
 *
 * @deprecated Intended for backwards compatibility support only.
 */
const ViewOtsFromSnipCliScreen = () => {
  ...
  const { id: secretId, token: secretPassword } = query as {
    id: string
    token: string
  }
    ...
    async function readSecret() {
      try {
        const requestOptions = {
          ...
          method: "post",
          body: JSON.stringify({
            OneTimeSecretId: secretId,
          }),
        }

        const response = await fetch(
          `https://api.sniptt.com/retrieveOneTimeSecret`,
          requestOptions,
        )
        ...
        const {
          OneTimeSecretEncryptedPrivateKey,
          OneTimeSecretEncryptedContent,
        }: OneTimeSecret = await response.json()

        // Decrypt OTS private key.
        const { data: otsPrivateKey } = await decrypt({
          message: await readMessage({
            armoredMessage: OneTimeSecretEncryptedPrivateKey,
          }),
          passwords: [secretPassword],
        })

        // Decrypt OTS.
        const { data: secretValue } = await decrypt({
          message: await readMessage({
            armoredMessage: OneTimeSecretEncryptedContent,
          }),
          decryptionKeys: await readPrivateKey({ armoredKey: otsPrivateKey }),
        })
...
export default withNoSsr(ViewOtsFromSnipCliScreen)
